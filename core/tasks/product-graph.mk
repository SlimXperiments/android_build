...
$(products_graph): $(this_makefile)
	@echo -e ${CL_CYN}"Product graph DOT:"${CL_RST}" $@ for $(PRIVATE_PRODUCTS_FILTER)
...
$(OUT_DIR)/products/$(strip $(1)).txt: $(this_makefile)
	@echo -e ${CL_CYN}"Product debug info file:"${CL_RST}" $$@"
	$(hide) rm -f $$@
	$(hide) mkdir -p $$(dir $$@)
...
$(call product-debug-filename, $(p)): \
			...
			$(this_makefile)
	@echo -e ${CL_CYN}"Product debug html file:"${CL_RST}" $$@"
	$(hide) mkdir -p $$(dir $$@)
